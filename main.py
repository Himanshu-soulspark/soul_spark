...
"""
================================================================
 Conceptra AI - मुख्य सर्वर फ़ाइल (main.py)
================================================================
...
def manage_tokens(uid, cost_in_tokens=1500):
    ...
    try:
        ...
        if not user_doc.exists:
            # अगर यूजर का प्रोफाइल नहीं है, तो एक बना दें।
            user_ref.set({'email': auth.get_user(uid).email, 'tokens_balance': 0})
            return False, {"error": f"आपका प्रोफाइल अभी बना है। इस काम के लिए {cost_in_tokens} टोकन चाहिए। कृपया रिचार्ज करें।"}
        ...
        print(f"SUCCESS: User {uid} spent {cost_in_tokens} tokens.")
        return True, None
    except Exception as e:
...
def check_user_privileges(uid, cost_in_tokens):
    """
    यूजर की पहचान करता है, मालिक को टोकन चेक से छूट देता है, और सामान्य यूजर के टोकन मैनेज करता है।
    """
    if not uid:
...
# Payment Routes (No changes needed)
@app.route('/create-order', methods=['POST'])
def create_order():
    if not razorpay_client: return jsonify({"error": "पेमेंट सेवा अभी उपलब्ध नहीं है।"}), 503
    data = request.get_json()
    amount, uid = data.get('amount'), data.get('uid')
    if not amount or not uid: return jsonify({"error": "राशि और यूजर आईडी आवश्यक हैं।"}), 400
    try:
        order = razorpay_client.order.create({
            "amount": int(amount) * 100, "currency": "INR",
            "receipt": f"receipt_{uid}_{int(time.time())}", "notes": {"firebase_uid": uid}
        })
...
def razorpay_webhook():
    ...
    if not webhook_secret: return 'Server configuration error', 500
    try:
        razorpay_client.utility.verify_webhook_signature(request.get_data(), request.headers.get('X-Razorpay-Signature'), webhook_secret)
    except Exception as e:
    ...
    if payload['event'] == 'payment.captured' and db:
        ...
        uid, amount_paid = payment_info['notes'].get('firebase_uid'), payment_info['amount']
        tokens_to_add = 0
        if amount_paid == 10000: tokens_to_add = 50000    # ₹100
        elif amount_paid == 45000: tokens_to_add = 250000  # ₹450
...
def ask_ai_image_route():
    ...
    if not is_authorized: return error_json, status_code
    if not model: return jsonify({'error': 'AI अभी अनुपलब्ध है।'}), 503
    question_text, image_file = request.form.get('question', ''), request.files.get('image')
    ...
    if image_file: prompt_parts.append(Image.open(image_file))
    if question_text: prompt_parts.append(f"\nUser's Question: {question_text}")
...
def generate_notes_route():
    ...
    if not model: return jsonify({'error': 'AI अभी अनुपलब्ध है।'}), 503
    data = request.get_json()
    topic, note_type = data.get('topic'), data.get('noteType', 'long')
    if not topic: return jsonify({'error': 'कृपया एक विषय प्रदान करें।'}), 400
    prompt = f'ROLE: Expert teacher. TASK: Generate {"brief summary and key bullet points" if note_type == "short" else "comprehensive, well-structured notes"} for "{topic}".\n{FORMATTING_INSTRUCTIONS}'
...
def generate_mcq_route():
    ...
    if not model: return jsonify({'error': 'AI अभी अनुपलब्ध है।'}), 503
    data = request.get_json()
    topic, count = data.get('topic'), min(int(data.get('count', 5)), 50)
    if not topic: return jsonify({'error': 'कृपया एक विषय प्रदान करें।'}), 400
...
def get_solved_notes_route():
    ...
    if not model: return jsonify({'error': 'AI अभी अनुपलब्ध है।'}), 503
    data = request.get_json()
    topic, count = data.get('topic'), min(int(data.get('count', 3)), 50)
    if not topic: return jsonify({'error': 'कृपया एक विषय प्रदान करें।'}), 400
...
def generate_flashcards_route():
    ...
    if not model: return jsonify({'error': 'AI अभी अनुपलब्ध है।'}), 503
    data = request.get_json()
    topic, count = data.get('topic'), min(int(data.get('count', 8)), 50)
    if not topic: return jsonify({'error': 'कृपया एक विषय प्रदान करें।'}), 400
